...
/*
* Check the alarm list to see if an alarm with this type has been assigned.
*
* return 1 if so and 0 otherwise.
*/
int check_duplicate(int type){
  int     status;
  alarm_t *next, **last;
  ...
  next = *last;
  while (next != NULL) {
    if(next->type == type && next->status == 1){
